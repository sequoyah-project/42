...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SQYTEST2\r");
            Send(hSocket, "WHO #SQYTEST2\r");
        } else {
            // randomly join #SQY00-#SQY05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SQY%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SQY%02d\r", channel_number).c_str());
        }
