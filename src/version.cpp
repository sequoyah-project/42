...
#ifdef GIT_ARCHIVE
#define GIT_COMMIT_ID "c404f53"
#endif

#define BUILD_DESC_FROM_COMMIT(maj,min,rev,commit) \
    "SQY-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-g" commit

#define BUILD_DESC_FROM_UNKNOWN(maj,min,rev) \
    "SQY-v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(rev) "." DO_STRINGIZE(build) "-unk"
